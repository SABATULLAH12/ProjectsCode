    ...
    public class Report : BaseChart, IReport
    {
        ...
        public DataTable GetTableOutputPPT(FilterPanelInfo[] filter, string spName)
        {
            ...
            try
            {
                try
                {
                    Log.LogMessage("SP :" + spName + Environment.NewLine);
                    Log.LogMessage("param :" + param[0] + "," + param[1] + "," + param[2] + "," + param[3] + "," + param[4] + "," + param[5] + Environment.NewLine);
                }
                catch(Exception e)
                {
                    Log.LogMessage("parameter Error " + e.Message + " " + e.StackTrace);
                }
                if (spName == "USP_SituationAssesmentReport_CoreGuest_HEC")
                {
            ...
            {
                Log.LogMessage(Environment.NewLine+"Execute DataSet "+e.Message+Environment.NewLine);
               
            }
            //var ds = database.ExecuteDataSet(cmd);

            Log.LogMessage(Environment.NewLine + " Sp executed " + Environment.NewLine);
            if (ds == null)
                return null;
            else
                return ds.Tables[0];
