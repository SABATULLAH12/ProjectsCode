...
angular.module("mainApp").controller("analysesController", ["$scope", '$q', "$http", function ($scope, $q) {
    ...
    $scope.showMoreLess = function () {
        if ($(".adv-fltr-showhide-txt").text() == "SHOW LESS") {
            ...
            $(".advance-filters").css("height", "80px");
    ...
    $scope.plotTableAnalyses = function (data) {
        ...
        /*-------------------------Patch code added to resolve scroll issue for IE Screen-----------------------*/
        $('.scrollable-columns-frame').css('padding-right', '2px');
        $('.scrollable-data-frame').css('padding-right', '2px');
        /*-----------------------------------------------------------------------------------------------------*/
        ...
        $(".expanse_collapse").unbind().click(function () {
            ...
            var x = $(this).attr("class").split(" ");
            var class_click = x[x.length - 1];
            if ($("." + class_click).parent().find(".dataShow").length != 0) {
                $(".dataShow").addClass(' dataHide').removeClass('dataShow');
                $("." + class_click).find(".tbl-data-expan-collapse_show").css('background-position', '-1510px -217px');
            }
            else if ($("." + class_click).parent().find('.dataHide').length != 0) {
                $(".dataHide").addClass(' dataShow').removeClass('dataHide');
                $("." + class_click).find(".tbl-data-expan-collapse_show").css('background-position', '-1554px -217px');
            }
    ...
    $scope.prepareReportParameters = function (reportName) {
        ...
        for (i = 0; i < filterPanelInfoData.filter.length; i++) {
            var data = filterPanelInfoData.filter[i];
            if (data.Name == "Time Period" && data.Name != null && data.Data != null && data.Data != undefined && data.SelectedID != undefined) {
                timePeriodType = data.SelectedID;
                fromTimePeriod = data.Data[0].Text.trim();
            }
        }
        ...
        if (reportName == undefined || reportName == null || reportName == "")
            var filterdata = { Filter: filterPanelInfo.filter, ChartType: $scope.activeChartType, Module: controllername, ReportName: $scope.reportSelected.Name, TimePeriodText: periodType, TimePeriodType: timePeriodType, FromTimePeriod: fromTimePeriod, ToTimePeriod: fromTimePeriod, Comment: "" };
        else if (reportName == "editedReport") {
            var filterdata = { Filter: filterPanelInfo.filter, ChartType: $scope.activeChartType, Module: controllername, SlideID: $scope.activeSlideID, ReportID: $scope.activeReportID, TimePeriodText: periodType, TimePeriodType: timePeriodType, FromTimePeriod: fromTimePeriod, ToTimePeriod: fromTimePeriod, Comment: $scope.commentText };
        }
        else
            var filterdata = { Filter: filterPanelInfo.filter, ChartType: $scope.activeChartType, Module: controllername, ReportName: $scope.reportName, TimePeriodText: periodType, TimePeriodType: timePeriodType, FromTimePeriod: fromTimePeriod, ToTimePeriod: fromTimePeriod, Comment: "" };
        var isVisits = 1;
        if ($('.adv-fltr-guest').css("color") == "rgb(255, 0, 0)")
            isVisits = 0;
        else
...
var validateFilter = function () {
    if (controllername == "analysescrossDinerFrequencies") {
        if ($(".Establishment_topdiv_element .sel_text").length == 0)
            showMaxAlert("Establishment is required. Please select");
        return false;
    }
