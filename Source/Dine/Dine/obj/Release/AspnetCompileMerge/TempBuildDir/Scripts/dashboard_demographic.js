...
var getDashboardData = function (filterPanelInfo) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".pie_charts .main._idballon1").remove(); $(".dashboard-content").show();
    ...
    { "name": "Time Spent In Outlet", "value": "none" }, { "name": "Beverage Item", "value": "none" }, { "name": "Food Item", "value": "none" }];
            ...
            BindAvgDollarSpent(retrnMetricList("Average Per Diner", dashboarddata));//Average Dollars Spent
            ...
            BindDataForFB_Consumed(retrnMetricList("Type of Visit", dashboarddata));//Append Food & Beverage Consumed
            Bind_time_spent_in_outlet(retrnMetricList("Time Spent In Outlet", dashboarddata));//Time Spent In Outlet
            Bind_avg_distance_travelled(retrnMetricList("Average Distance Travelled", dashboarddata));//Average Distance Travelled
            Bind_mode_of_transport(retrnMetricList("Mode of Transport", dashboarddata));//Mode of Transport
            Bind_location_prior(retrnMetricList("Location Prior", dashboarddata));//Location Prior
            Bind_planning_type(retrnMetricList("Planning Type", dashboarddata));//Planning Type
            Bind_companion_detail(retrnMetricList("Companion Detail", dashboarddata));//Companion Detail
            ...
            Bind_destination_after_visit(new_list);
            Bind_beverage_item(retrnMetricList("Beverage Item", dashboarddata));//To Bind top Beverage Item
            Bind_food_item(retrnMetricList("Food Item", dashboarddata));//To Bind top Food Item
            Bind_primary_occasion_motvation(retrnMetricList("Primary Occasion Motvation", dashboarddata));//To Bind Occasion Motivation
            Bind_primary_outlet_motivation(retrnMetricList("Primary Outlet Motivation", dashboarddata));//To Bind Primary Outlet Motivation
            Bind_primary_mood(retrnMetricList("Primary Mood", dashboarddata));//To Bind Primary Outlet Motivation
            ...
            $('.establishmt_img').css("background-image", 'url("../Images/P2PDashboardEsthmtImages/' + seltdestablismt.replace(/\//g, "") + '.svg")');//replace("/","")
...
$(document).on("click", ".pdf-logo", function () {
    ...
    $.ajax({
            ...
            data: JSON.stringify({ "OutputData": dashboarddata, "NoOfRoads": (($("#p2p-dsbrd-BG object").attr("data")).indexOf("BG2") == -1 ? "4" : "3"), "changedData": dynamicChanges, "LeftpanelData": leftpanelData, "statTest": statTest, "pptOrPdf": "pdf", "ss": ss }),
            method: "POST",
            contentType: "application/json",
...
$(document).on("click", ".ppt-logo", function () {
    ...
    $.ajax({
        url: appRouteUrl + "DashBoard/ExportToFullDashboardPPT",
        data: JSON.stringify({ "OutputData": dashboarddata, "NoOfRoads": (($("#p2p-dsbrd-BG object").attr("data")).indexOf("BG2") == -1 ? "4" : "3"), "changedData": dynamicChanges, "LeftpanelData": leftpanelData, "statTest": statTest, "pptOrPdf": "ppt", "ss": ss }),
        method: "POST",
        contentType: "application/json",
        success: function (response) {
            if (response != "error")
...
$(document).on("click", ".popup-ppt", function () {
    ...
    $.ajax({
        ...
        async: true,
        data: JSON.stringify({ "DemofilterName": ($(".inner_submit").attr("data-val")), "OutputData": OutputData, "LeftpanelData": leftpanelData, "statTest": statTest, "pptOrPdf": "ppt", "DemoTitle": $(".popup-hdrtxt").text().toUpperCase().trim(), "ss": ss  }),
        method: "POST",
        contentType: "application/json",
        success: function (response) {
            if (response != "error")
...
$(document).on("click", ".popup-hdrpdf", function () {
    ...
    $.ajax({
        url: appRouteUrl + "DashBoard/PopupExportDashboard",
        async: true,
        data: JSON.stringify({ "DemofilterName": ($(".inner_submit").attr("data-val")), "OutputData": OutputData, "LeftpanelData": leftpanelData, "statTest": statTest, "pptOrPdf": "pdf", "DemoTitle": $(".popup-hdrtxt").text().toUpperCase().trim(), "ss": ss }),
        method: "POST",
        contentType: "application/json",
        success: function (response) {
            if (response != "error")
...
$(document).on("click", ".plus_icon_click", function () {
    ...
    DataArr.forEach(function (d, i) {
        ...
        if (dataval != "Food Item") {
            if (dataval == "Overall Satisfaction (Top 2 Box)" || dataval == "Trip Activities" || dataval == "Distance Travelled") {
                //For Pie Charts Popup only
                if (i % 2 == 0)
                    dashbordTopFoodItem += '<div class="bar-content-main-div"><div class="bar-cntnt-metric-brdr  onlyfor-without-rdobtns"></div><div class="bar-cntnt-metric-name onlyforpie-metricname">' + d.MetricName + '</div><div class="bar-cntnt-metric-div onlyforPie backgn-clr"><div class="bar-cntnt-innerdiv"><div class="bar-cntnt-outer"><div class="bar-cntnt-inner"><div class="bar-cnt-inner-dashdiv"> </div><div class="bar-cnt-inner-metrc-percnt" style="width:' + reSize + '%"></div></div></div></div><div class="bar-cnt-left-metrc-val-brdr"></div><div class="bar-cntnt-metric-val" style="color:' + signcolor + '">' + (d.MetricValue == undefined ? 0 : d.MetricValue.toFixed(0)) + '%</div><div class="bar-cntnt-brder" style="background-color:' + signcolor + '"> </div><div class="bar-cntnt-change-val" style="color:' + signcolor + '">' + getChangeVal(d.Change) + '</div></div></div></div></div>';
                else
            ...
            //if (i < 6) {
                if (j == 0) {
                    ...
                    dashbordTopFoodItem += '<div class="bar-content-left"><div class="bar-cntnt-radiodivleft"><div id="radio-click" class="bar-cntnt-radioimg marginforradiotop" data-val="' + d.MetricName + '"></div></div><div class="bar-cntnt-metric-brdr topfdbrdr"></div><div class="bar-cntnt-metric-name top-fditem-lnehght">' + d.MetricName + '</div><div class="bar-cntnt-metric-val" style="color:' + signcolor + '">' + (d.MetricValue == undefined ? 0 : d.MetricValue.toFixed(0)) + '%</div><div class="bar-cntnt-brder" style="background-color:' + signcolor + '"> </div><div class="bar-cntnt-change-val topfditemchange" style="color:' + signcolor + '">' + getChangeVal(d.Change) + '</div></div>';
                    
                    j++;
                }
                else {
                    dashbordTopFoodItem += '<div class="bar-content-left"><div class="bar-cntnt-radiodivleft"><div id="radio-click" class="bar-cntnt-radioimg marginforradiotop" data-val="' + d.MetricName + '"></div></div><div class="bar-cntnt-metric-brdr topfdbrdr"></div><div class="bar-cntnt-metric-name top-fditem-lnehght">' + d.MetricName + '</div><div class="bar-cntnt-metric-val" style="color:' + signcolor + '">' + (d.MetricValue == undefined ? 0 : d.MetricValue.toFixed(0)) + '%</div><div class="bar-cntnt-brder" style="background-color:' + signcolor + '"> </div><div class="bar-cntnt-change-val topfditemchange" style="color:' + signcolor + '">' + getChangeVal(d.Change) + '</div></div></div></div></div></div>';
                    j = 0; 
                }
                if (d.MetricName == subtext) {
...
var Bind_destination_after_visit = function (new_list) {
    ...
    else if (new_list[0].MetricName == "Someone else’s home")
...
var Bind_time_spent_in_outlet = function (data) {
    $(".Time_Spent").show();
...
//Location Prior
var Bind_location_prior = function (data) {
    $(".Location_Prior").show();
...
//Planning Type
var Bind_planning_type = function (data) {
    //clear the div
    $(".Planning_Type .p_pieChart").html('');
    $(".Planning_Type").show(); $(".Planning_Type .p_img").hide();
    ...
    if ("SPUR OF THE MOMENT" == data[0].MetricName.toUpperCase()) {
        ...
        $(".Planning_Type .p_img").css("background-image", "url('../Images/P2PDashboardImages/P2P_Planning_" + Math.ceil(data[0].MetricValue / 20) + ".svg')");
    }
    
}
//Companion Detail
var Bind_companion_detail = function (data) {
...
//Mode of Transport
var Bind_mode_of_transport = function (data) {
...
var Bind_overall_satisfaction_top___box = function (data) {
...
var BindAvgDollarSpent = function (data) {
...
var retrnMetricList = function (demofiltername, dashboarddata) {
    ...
    if (demofiltername == "Satisfaction") {
        $.each(dashboarddata, function (i, v) {
            if (v.DemofilterName == "Satisfaction: Food (Top 2 Box)" || v.DemofilterName == "Satisfaction: Beverages (Top 2 Box)" || v.DemofilterName == "Satisfaction: Value (Top 2 Box)" || v.DemofilterName == "Satisfaction: Cleanliness (Top 2 Box)" || v.DemofilterName == "Satisfaction: Atmosphere (Top 2 Box)" || v.DemofilterName == "Satisfaction: Service (Top 2 Box)")
                outputlist.push({ MetricValue: v.MetricValue, MetricName: v.MetricName, Significancevalue: v.Significancevalue, Change: v.Change, Skew: v.Skew });
        });
    } 
    else {
        $.each(dashboarddata, function (i, v) {
            if (v.DemofilterName == demofiltername)
                outputlist.push({ MetricValue: v.MetricValue, MetricName: v.MetricName, Significancevalue: v.Significancevalue, Change: v.Change, Skew: v.Skew, EstablishmentName: v.EstablishmentName });
        });
...
var Bind_beverage_item = function (data) {
    ...
    $.each(data, function (i, v) {
        ...
        $('.bevg-change-sign').html(getChangeVal(v.Change));
...
var Bind_food_item = function (data) {
    ...
    $.each(data, function (i, v) {
        ...
        //var changeOrSkew = v.Change == undefined || v.Change == null ? "0.0%" : v.Change > 0 ? "+" + v.Change.toFixed(1) + "%" : v.Change.toFixed(1) + "%";
        $('.food-change-sign').html(getChangeVal(v.Change));
