...
app.controller("tableController", ["$rootScope", "$scope", '$q', "$http", "$timeout", function ($rootScope, $scope, $q, $timeout) {
    ...
    $scope.showMoreLess = function () {
        if ($(".adv-fltr-showhide-txt").text() == "SHOW LESS") {
            ...
            $(".advance-filters").css("height", "145px");
    ...
    $scope.plotTableData = function (tabledata, columns, sampleSizeList) {
        ...
        $.each(tabledata, function (i, v) {
            ...
            if (mainHeaderList.indexOf(v[0].DemoFilterName) == -1) {
                ...
                for (var k = 0; k < columns.length; k++) {
                    if (k == 0) {
                        ...
                        //bug id 4860
                        tbodyHtml += '<div class="tbl-data-brderbtm"></div><div class="tbl-dta-demoflter"><div class="middleAlignHeader leftAlignTableText">' + v[0].DemoFilterName + '</div></div></td><td class="emptydiv"><div class="tbl-shadow">&nbsp;</div></td>';
                        tbodyHtml += '<td class="tbl-dta-td tbl-dta-rowscolr ' + removeBlankSpace(columns[k]) + '_hide"></td><td class="emptydiv ' + removeBlankSpace(columns[k]) + '_hide"><div class="tbl-shadow">&nbsp;</div></td>';
                    }
                    else {
                        tbodyHtml += '<td class="tbl-dta-td tbl-dta-rowscolr ' + removeBlankSpace(columns[k]) + '_hide">&nbsp;</td><td class="emptydiv ' + removeBlankSpace(columns[k]) + '_hide"><div class="tbl-shadow">&nbsp;</div></td>';
                    }
                ...
                if (($(".box.adv-fltr-label[style='color: rgb(255, 255, 255);']").attr("data-id") == "5" && ($(".master-lft-ctrl[data-val='CONSUMED FREQUENCY']").find(".lft-popup-ele-label[data-val='Favorite Brand In Category']").parent().hasClass("lft-popup-ele_active") || $(".master-lft-ctrl[data-val='CONSUMED FREQUENCY']").find(".lft-popup-ele-label[data-val='Favorite Brand Across NAB']").parent().hasClass("lft-popup-ele_active"))) || ($(".box.adv-fltr-label[style='color: rgb(255, 255, 255);']").attr("data-id") == "8") || ((filename == "BeverageCompare" || filename == "BeverageDeepDive") && $(".box.adv-fltr-label[style='color: rgb(255, 255, 255);']").attr("data-id") == "7")) {
                    ...
                    for (var j = 0; j < columns.length; j++) {
                        ...
                        $.each(sampleSizeList, function (sampleIndex, sampleValue) {
                            ...
                            if (columns[j].toString().toUpperCase().trim() == sampleValue.EstablishmentName.toString().toUpperCase().trim()
                                && sampleValue.DemoFilterName == v[0].DemoFilterName) {
                        ...
                        //if (columns[j] == v[0].EstablishmentName) {
            ...
            //for (var j = 0; j < columns.length; j++) {
            if ($(".box.adv-fltr-label[style='color: rgb(255, 255, 255);']").attr("data-id") == "8" || ((filename == "BeverageCompare" || filename == "BeverageDeepDive") && $(".box.adv-fltr-label[style='color: rgb(255, 255, 255);']").attr("data-id") == "7")) {
                    ...
                    if (vValue.EstablishmentName == columnValue.EstablishmentName) {
                        ...
                        if (columnValue.MetricSampleSize == null || columnValue.MetricSampleSize == "") {
                            ...
                            if (columnValue.MetricSampleSize >= 30 && columnValue.MetricSampleSize <= 99) {
                                ...
                                cssForUseDirectnlyyCircle = "yellowcolorcircle"; cssForUseDirectnlytd = "yellowcolortd";
        ...
        if (!isCustomBaseSelect) {
            ...
            if (!$('#table-ttldine').hasClass("activestat")) {
                ...
                $(".scrollable-columns-table tr th").each(function (i) {
                    if ($(this).children("div").html() == actcusname) {
                        cusindx = i;
                        return false;
                    }
                });
    ...
    function defaultFreqncySelctn() {
        return $q(function (resolve, reject) {
            setTimeout(function () {
                ...
                var topFilterListVisits = ["2", "3", "4", "7"];
                if ($('#guest-visit-toggle').hasClass('activeToggle')) {
                    if ($(".adv-fltr-sub-frequency .master-lft-ctrl[data-val='FREQUENCY']").first().find(".lft-popup-ele-label[data-val='Monthly+']").parent().hasClass("lft-popup-ele_active") && isInitialLoad) {
                        resolve('Successful');
                    } else {
                        if (isInitialLoad == false) {
                            if ($(".adv-fltr-sub-frequency .master-lft-ctrl[data-val='FREQUENCY']").find(".lft-popup-ele_active").length == 0) {
                                ...
                                else if (selctedTopFilterId == "8" && selectedFrequencyToCarryFrwd == "Monthly+") {
                                ...
                                else if (selectedFrequencyToCarryFrwd_flag == 0 && selectedFrequencyToCarryFrwd == "Monthly+")
                                {
                                    $(".adv-fltr-sub-frequency .master-lft-ctrl[data-val='FREQUENCY']").first().find(".lft-popup-ele-label[data-val='Monthly+']").click();
                                }
                                else {
...
var removeBlankSpace = function (object) {
    object = object.trim();
    //var text = object.replace(/\ /g, "_").replace(/\//g, "").replace(/\(/g, "").replace(/\)/g, "").replace(/\&/g, "_").replace(/\%/g, "").replace(/\./g, "").replace(/\-/g, "_").replace(/\,/g, "_").replace(/\|/g, "").replace(/\:/g, "_").replace(/\,/g, "_").replace(/[0-9]+/, "_").replace(/\'/g, '').replace(/\"/g, '').replace(/\+/g, '').replace(/\$/g, '').replace(/\+~!@#$/g, '');
    var text = object.replace(/\ /g, "_").replace(/\//g, "").replace(/\(/g, "").replace(/\)/g, "").replace(/\&/g, "_").replace(/\%/g, "").replace(/\./g, "").replace(/\-/g, "_").replace(/\,/g, "_").replace(/\|/g, "").replace(/\:/g, "_").replace(/\,/g, "_").replace(/\'/g, '').replace(/\"/g, '').replace(/\+/g, '').replace(/\$/g, '').replace(/\+~!@#$/g, '').replace(/\!/g,'');;
    return text.toLowerCase();
}
...
var emptyTableoutputWithSelectedColumns = function () {   
   ...
   tableFrequncydiv += '</div>';
    ...
    $(".table-bottomlayer").css("margin-top", "10px");
