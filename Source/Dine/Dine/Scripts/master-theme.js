...
var additionalfilterListVisit = ["Daypart", "Day of Week", "Planning Type", "Service Mode", "Meal Type", "Visit Type", "Outlet Segments", "Food Items", "Beverage Consumed", "NARTD", "MEAL MISSION TRIP SEGMENT" ];
...
var CustomRegionsToolTipParentList = ["CCNA Regions", "Census Regions", "FCL Ops OU", "Walgreens Trade Areas"]
...
$(document).on("click", ".lft-popup-ele-label", function (e) {
    ...
    if ($(this).parent().hasClass('lft-popup-ele_active')) {
        ...
        if ($(this).attr('parent-text') == "Macro Meal Missions" || $(this).attr('parent-text') == "Micro Meal Missions Level 1"
            || $(this).attr('parent-text') == "Micro Meal Missions Level 2") {
            validationToSelectDataWithinSameParentTrip(this, $(this).attr('parent-text'));
        }
        if ($(this).parent().parent().attr('first-level-selection') == "Macro Meal Missions" || $(this).parent().parent().attr('first-level-selection') == "Micro Meal Missions Level 1"
            || $(this).parent().parent().attr('first-level-selection') == "Micro Meal Missions Level 2") {
            validationToSelectDataWithinSameParentTripAdv(this);
        }
                    ...
                    for (var j = 0; j < left_Menu_Data[data_index].PanelPopup[i].Data.length; j++) {
                        ...
                        if (obj.IsSelectable != undefined && obj.IsSelectable) {
                            if (controllername == "dashboard_demographics") {
                                ...
                                else if ((controllername == "dashboardp2pdashboard" || controllername == "dashboard_demographics" || controllername == "reportp2preport" || controllername == "situationassessmentreport")&& left_Menu_Data[data_index].Label == "Advance Filters")
                                {
                                    searchArray.push({ label: obj.SearchName, value: [{ filterobj: ele, level: left_Menu_Data[data_index].PanelPopup[i].Level, parentid: obj.ParentID, id: obj.ID }] });
                                }
                                else
...
var validationToSelectDataWithinSameParentTrip = function (key) {    
    var selectedEle = $(key).parent().parent().find(".lft-popup-ele_active");
    var level = $(key).attr('data-parent');
    $.each(selectedEle, function (index, ele) {
        var eleLevel = $(ele).find('.lft-popup-ele-label').attr('data-parent')
        if (level != eleLevel) {
            var Redlen = $(ele).find('[parent-text="Micro Meal Missions Level 2"]').length;
            var Greenlen = $(ele).find('[parent-text="Micro Meal Missions Level 1"]').length;
            var Bluelen = $(ele).find('[parent-text="Macro Meal Missions"]').length;

            if (Redlen == 0 && Greenlen == 0 && Bluelen == 0) {
...
var validationToSelectDataWithinSameParentTripAdv = function (key) {
    var selectedEle = $(key).parent().parent().find(".lft-popup-ele_active");
    var level = $(key).attr('data-parent');
    $.each(selectedEle, function (index, ele) {
        var eleLevel = $(ele).find('.lft-popup-ele-label').attr('data-parent')
        ...
        select: function (event, ui) {
            ...
            if (label == "Measures" && (window.location.pathname).replace("Dine/", "").split("/")[1].toUpperCase() == "CHART") {
                ...
                if (label == "Beverage" && listofOthers.indexOf(ui.item.value[0].id) > -1)
                    ...
                    lbserachparent = $('.master-lft-ctrl[data-val="' + label + '"]').find('.lft-popup-ele-label[data-isselectable=true][data-id="' + ui.item.value[0].id + '"]').first();
                else if (label == "Advance Filters" && (controllername == "dashboardp2pdashboard" || controllername == "dashboard_demographics" || controllername == "reportp2preport" || controllername == "situationassessmentreport"))
                    lbserachparent = $('.master-lft-ctrl[data-val="' + label + '"]').find('.lft-popup-ele-label[data-isselectable=true][data-id="' + ui.item.value[0].id + '"]').first();
                else
                    lbserachparent = $('.master-lft-ctrl[data-val="' + label + '"]').find('.lft-popup-ele-label[data-isselectable=true][data-val="' + selectedValue + '"]').first();
            }
...
var globallocalstoragecache = "300.21";
