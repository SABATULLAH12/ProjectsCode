...
var gettabledatage = [];
var currentSelectedTypeCount = 0;
var currentSelectedPyramidTypeCount = 0;
...
colorPyramidStart = ["#E41E2B", "#31859C", "#FFC000", "#00B050", "#7030A0", "#7F7F7F", "#C00000", "#0070C0", "#FF9900", "#D2D9DF", "#000000", "#838C87", "#83E5BB"];
colorPyramidStop = ["#AE192C", "#2E6878", "#936C0D", "#037C37", "#4D1F77", "#515151", "#820A0B", "#0A517C", "#C5790F", "#818284", "#000000", "#3b423e", "#4d886e"];
...
angular.module("mainApp").controller("chartController", ['$scope', '$q', '$http', function ($scope, $q) {
    ...
    function buildMenu() {
        var menuData = null;//clientDataStorage.get(controllername)
        if (menuData == null) {
            $.ajax({
    ...
    $scope.prepareReportParameters = function (reportName) {
        ...
        for (l = 0; l < cmt.length; l++) {
            if ($(cmt[l]).attr("data-val") != "Guest_or_Visits") {
                    ...
                    if ($(cmt[l]).attr("data-val").toUpperCase() == "FREQUENCY") {
                        if (labels.length > 0) {
                            if (isVisits == 1 && $(labels[0]).text().trim().toUpperCase() != "TOTAL VISITS") {
                                fil = fil.concat(isTotalUsPopulation == "Establishment Frequency (Base: Total US Population)" ? "NA" : "Total Visits ( " + $(labels[0]).text() + " )");
                            } else {
                                fil = fil.concat(isTotalUsPopulation == "Establishment Frequency (Base: Total US Population)" ? "NA" : $(labels[0]).text());
                            }
                        } else {
                        ...
                        if ($(cmt[l]).attr("data-val").toUpperCase() == "FREQUENCY") {
                            fil = fil.concat("Consumed Frequency : ");
                        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.Series == null || response.Series.length == 0) {
                    ...
                    let isPyramidThere = false;
                    $.each(data, function (i, v) {
                        if (i == 0) {
                            chartTypeList = v.data[i].ChartType;
                            if (chartTypeList.toLocaleLowerCase().indexOf("pyramidwithchange") != -1 || chartTypeList.toLocaleLowerCase().indexOf("pyramid") != -1) {
                                isPyramidThere = true;
                            }
                    ...
                    //Added by abhay singh (on 31-10-2017)//
                    /*------------------------------------- resetting to default color codes ---------------------------------------*/
                    if (active_chart_type != "pyramidwithchange" && active_chart_type != "pyramid" && !isPyramidThere) {
                        colorColumnStop = ["#AE192C", "#2E6878", "#936C0D", "#037C37", "#4D1F77", "#515151", "#820A0B", "#0A517C", "#C5790F", "#818284", "#000000", "#ffffff", "#e55330", "#875288", "#f89e16", "#69898d", "#988878", "#d8cec2", "#faba3a", "#c94c26", "#352414", "#67ac53", "#5f78a4"];
                        colorColumnStart = ["#E41E2B", "#31859C", "#FFC000", "#00B050", "#7030A0", "#7F7F7F", "#C00000", "#0070C0", "#FF9900", "#D2D9DF", "#000000", "#838C87", "#83E5BB", "#cccccc", "#b42c14", "#643160", "#be6e14", "#406462", "#605f4f", "#a3978b", "#c08617", "#9d270e", "#170909", "#368130", "#378574"];

                        var palatteType = getDataValforColor();
                        var selectedTypeStr = GetSelectedIds(filterPanelInfo.filter, palatteType);
                        var selectedTypeIds = (selectedTypeStr != undefined && selectedTypeStr != "") ? selectedTypeStr.split('|') : [];
                        currentSelectedTypeCount = selectedTypeIds.length;
                        let t_colorColumnStart = [], t_colorColumnStop = [], q = 0;
                        var p;
                        for (p = 0; p < selectedTypeIds.length; p++) {

                            var _colorCode = $('.master-lft-ctrl[data-val="' + palatteType + '"] span.lft-popup-ele-label[data-id="' + selectedTypeIds[p] + '"]').attr("colorcode");
                            if (_colorCode != undefined && _colorCode != "" && _colorCode != "null" && _colorCode != "NULL" && _colorCode != "Null" && _colorCode != null) {
                                t_colorColumnStart.push(_colorCode);
                                t_colorColumnStop.push(ColorLuminance(_colorCode, -0.20));
                            }
                            else {
                                t_colorColumnStart.push(colorColumnStart[q]);
                                t_colorColumnStop.push(colorColumnStop[q]);
                                q++;
                            }
                        ...
                        I = p;
                        J = q;
                        while (p < colorColumnStart.length) {
                            t_colorColumnStart.push(colorColumnStart[q++]);
                            p++;
                        }
                        while (I < colorColumnStop.length) {
                            t_colorColumnStop.push(colorColumnStop[J++]);
                            I++;
                        }
                        colorColumnStart = t_colorColumnStart;
                        colorColumnStop = t_colorColumnStop;
                    }
                    else {
                        ...
                        /////////////---------- Establishment --------------/////////////
                        palatteType = (controllername == "chartestablishmentdeepdive") ? "Metric Comparisons" : "Establishment";
                        selectedTypeStr = GetSelectedIds(filterPanelInfo.filter, palatteType);
                        selectedTypeIds = (selectedTypeStr != undefined && selectedTypeStr != "") ? selectedTypeStr.split('|') : [];
                        currentSelectedPyramidTypeCount = selectedTypeIds.length;
                        t_colorColumnStart = [], t_colorColumnStop = [], q = 0;
                        ...
                        for (p = 0; p < selectedTypeIds.length; p++) {
                            ...
                            if (_colorCode != undefined && _colorCode != "" && _colorCode != "null" && _colorCode != "NULL" && _colorCode != "Null" && _colorCode != null) {
                                ...
                                t_colorColumnStart.push(colorPyramidStart[q]);
                                t_colorColumnStop.push(colorPyramidStop[q]);
                                q++;
                            }
                        ...
                        I = p;
                        J = q;
                        while (p < colorPyramidStart.length) {
                            t_colorColumnStart.push(colorPyramidStart[q++]);
                            p++;
                        }
                        while (I < colorPyramidStop.length) {
                            t_colorColumnStop.push(colorPyramidStop[J++]);
                            I++;
                        }
                        colorPyramidStart = t_colorColumnStart;
                        colorPyramidStop = t_colorColumnStop;
    ...
    var GetSelectedIds = function (filters, fType) {
        for (var i = 0; i < filters.length; i++) {
            if (filters[i]['Name'] == fType)
                return filters[i]['SelectedID'];
        }
        return "";
    }
    ...
    $scope.closeColorPopup = function () {
        ...
        $(".color-pallet-chart").css("background-position", "-2228px -408px");
    }
    $scope.createReport = function () {
        if ($scope.reportName == "") {
            showMaxAlert("Please name the report");
        }
        else {
            if ($scope.reportList.length > 0) {
                ...
                for (i = 0; i < $scope.reportList.length; i++) {
                    ...
                    if (rpt.Name.toLowerCase() == $scope.reportName.toLowerCase()) {
                        $scope.IsNameExist = 1;
                        showMaxAlert("Report with the same name already exist");
                    }
                }
    ...
    $scope.addSlideToReport = function (Id) {
        if (Id == undefined) {
            showMaxAlert("Please select/create report");
            return false;
        }
    ...
    $scope.showMoreLess = function () {
        if ($(".adv-fltr-showhide-txt").text() == "SHOW LESS") {
            ...
            $(".adv-fltr-showhide-sectn").css("margin-top", "-30px");
            $(".advance-filters").css("height", "30px");
            $(".adv-fltr-details").css("height", "30px");
            $(".adv-fltr-headers").css("height", "33px");
            $(".chart-bottomlayer").css("height", "calc(93% - 104px)");
            $(".adv-fltr-showhide").css({ "margin-top": "-5px" });
            $(".adv-fltr-applyfiltr").css("visibility", "hidden");
            $(".adv-fltr-showhide").css({ "top": "0px" });
            ...
            $(".adv-fltr-showhide-sectn").css("height", "28px");
            $(".adv-fltr-showhide-sectn").css("margin-top", "-69px");
            $(".advance-filters").css("height", "76px");
            $(".adv-fltr-details").css("height", "70px");
            $(".adv-fltr-headers").css("height", "33px");
            $(".adv-fltr-applyfiltr").css("visibility", "hidden");
            $(".chart-bottomlayer").css("height", "calc(93% - 190px)");
    ...
    $scope.plotTableChart = function (data) {
        ...
        if (tree[2][0].x == 'Satisfaction Drivers - Top 2 box' == true) {
            if ($(".lft-ctrl-toggle-text.pit.active").length == 0 && (controllername == "chartestablishmentdeepdive" || controllername=="chartestablishmentcompare" || controllername=="chartbeveragecompare" || controllername=="chartbeveragedeepdive")) {}
            else{
                var isSatisTop2Box = true;
                isSatisfactionDrivers = true;
                tree.splice(1, 1);
            }
        }

        //var isSatisTop2Box = ((tree[2][0].x == 'Satisfaction Drivers - Top 2 box') == true) ? true : false;
        //if (isSatisTop2Box==true)
        //    if($(".lft-ctrl-toggle-text.pit.active").length==1 && controllername!="chartestablishmentdeepdive"){}
        //        tree.splice(1, 1);
        var index = (isSatisTop2Box) ? 1 : 2;
        tBody += '<tbody>';
        for (i = 1; i < tree.length; i++) {
            if (i == 1 && !isSatisTop2Box) {
                ...
                if (isSatisTop2Box) {
                    ...
                    tBody += '<tr class="tbl-dta-rows dataShow"><td class="total_visits"><div class="tbl-algn tbl-algn-left"><div class="text_middle">SAMPLE SIZE</div></div><div class="tbl-data-btmbrd"></div><div class="tbl-btm-circle"></div></td>';
                    for (var k = 1; k < arrayLength ; k++) {
                        tBody += '<td class="emptydiv"><div class="tbl-shadow">&nbsp;</div></td><td><div class="tbl-algn "><div class="text_middle" style="color:Black;">' + sampleSizeStatus(tree[i][k].MetricSampleSize) + '</div></div><div class="tbl-data-btmbrd"></div><div class="tbl-btm-circle "></div></td>';
                    }
                    tBody += '</tr>';
               }
            ...
            for (j = 0; j < tree[i].length; j++) {
                var checkSample = tree[i][j].x.split(" ")[0];
                var ssColorCSS = (checkSample >= 30 && checkSample < 90) ? "greycolorusedirectnlly" : "";
                ...
                if (i == index && j == 0) {
                    ...
                    if (IstrendTableNewStructure != true && tree[i][j].x == "Sample Size")
                        tBody += '<td class="' + tree[i][j].css + ' ' + tree[i][j].dataCss + ' ' + tree[i][j].useDirectionallyFortdCSS + ' ' + ssColorCSS + ' ' + (i == 1 ? "" : removeBlankSpace(tree[0][j].x)) + '">' +
                    ...
                    var usedirectionallyforIndividualrows = "";
                    if (isSatisfactionDrivers) {
                        if (tree[i][j].MetricSampleSize != null && tree[i][j].MetricSampleSize != "") {
                            if (tree[i][j].MetricSampleSize >= 30 && tree[i][j].MetricSampleSize <= 99)
                                usedirectionallyforIndividualrows = "grayColorForSatisfactnDrvers";
                            else if(tree[i][j].MetricSampleSize < 30)
                                usedirectionallyforIndividualrows = "hideForSatisfactnDrvers";
                            else if (tree[i][j].MetricSampleSize > 99) {
                                if (tree[i][j].dataCss != "green" && tree[i][j].dataCss != "red")
                                    usedirectionallyforIndividualrows = "blackColorForSatisfactnDrvers";
                                }                            
                        }
                    }
                    tBody += '<td class="' + tree[i][j].css + ' ' + tree[i][j].dataCss + ' ' + tree[i][j].useDirectionallyFortdCSS + ' ' +   usedirectionallyforIndividualrows +  ' ' + (i == 1 ? "" : removeBlankSpace(tree[0][j].x)) + '">' +
                     '<div class="tbl-algn "><div class="text_middle">' + tree[i][j].x + '</div></div>';
                }
    ...
    //------- Function updated by Abhay Singh on (30-10-2017) --------//
    $scope.customcolrSubmit = function () {
        ...
        for (var i = 0; i < colorCodeObjs.length; i++) {
            ...
            if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") {
                colorPyramidStart[i] = colorCodeObjs[i].color;
                colorPyramidStop[i] = ColorLuminance(colorCodeObjs[i].color, -0.20);  //'-0.20' means 20% darker
            }
            else {
                colorColumnStart[i] = colorCodeObjs[i].color;
                colorColumnStop[i] = ColorLuminance(colorCodeObjs[i].color, -0.20);  //'-0.20' means 20% darker
            }

            if (colorCodeObjs[i].color != colorCodeObjs[i].originalcolor) {
                if (palatteType == "Measures") {
                    var colorCodeObj = {
                        ColourCode: colorCodeObjs[i].color,
                        Establishmentid: '',
                        MeasureId: colorCodeObjs[i].id,
                        GroupsId: '',
                        IsTrend: IsPIT_TREND
                    };
                    changedColorCodeList.push(colorCodeObj);
                }
                else if (palatteType == "Establishment") {
                    var colorCodeObj = {
                        ColourCode: colorCodeObjs[i].color,
                ...
                else if (palatteType == "Metric Comparisons") {
                    var colorCodeObj = {
                        ColourCode: colorCodeObjs[i].color,
                        Establishmentid: '',
                        MeasureId: '',
                        GroupsId: colorCodeObjs[i].id,
                        IsTrend: IsPIT_TREND
                    };
                    changedColorCodeList.push(colorCodeObj);
                }
                else if (palatteType == "Beverage") {
                    var colorCodeObj = {
                        ColourCode: colorCodeObjs[i].color,
                        Establishmentid: colorCodeObjs[i].id,
                        MeasureId: '',
        ...
        $(".transparentBG").show();
        $.ajax({
            url: appRouteUrl + "Chart/SaveChangedColorCode",
            ...
            success: function (response) {
                ...
                if (IsPIT_TREND == false) {
                    if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") {
                        $scope.pyramidwithchange($scope.chartData);
                    }
                    else {
                        $scope.column($scope.chartData); //TO DO: load active chart type only
                    }
                }
                else
                    $scope.plotTrendChart($scope.chartData);
    ...
    $scope.saveComment = function () {
        if ($scope.commentText == "") {
            showMaxAlert("Please add Comment to the slide");
        }
        else {
            $(".loader").show();
    ...
    $(document).on("click", '.ppt-logo', function () {
       ...
       // if ($(".chrt-typ.active").hasClass("trend-chart") || $(".chrt-typ.active").hasClass("stackedbar-chart") || $(".chrt-typ.active").hasClass("stackedcolumn-chart") || $(".chrt-typ.active").hasClass("bar-chart") || $(".chrt-typ.active").hasClass("barwithchange-chart") || $(".chrt-typ.active").hasClass("column-chart") || $(".chrt-typ.active").hasClass("pyramid-chart") || $(".chrt-typ.active").hasClass("pyramidwithchange-chart")) {
            ...
            let _changedColors = [];
            if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid")
                _changedColors = colorPyramidStart.slice(0, currentSelectedPyramidTypeCount);
            else
                _changedColors = colorColumnStart.slice(0, currentSelectedTypeCount)

            var data = { filter: $scope.prepareReportParameters("reportName"), changedColors: _changedColors };
            //data.filter.Filter[0].DemoAndTopFilters = fil;
        ...
        //} else {
        //    showMaxAlert("Please select chart only");
        //}
    });
    $(document).on("click", '.exl-logo', function () {
        ...
        if ($(".chrt-typ.active").hasClass("table-chart")) {
            ...
            //
            var advanceFiltersList = ["DEVICE USED", "DAYPART", "DAY PART", "DAY OF WEEK", "BEVERAGE ITEMS", "FOOD ITEMS", "Establishment", "Planning Type", "Service Mode", "Meal Type", "Visit Type", "Outlet Segments", "Beverage Consumed", "MEAL MISSION TRIP SEGMENT" ];
            ...
            showMaxAlert("Please select table only");
        }
    });
        ...
        return $q(function (resolve, reject) {
            setTimeout(function () {
                if (isVisitsSelected_Charts == 0) {
                    if ($(".adv-fltr-sub-frequency .master-lft-ctrl[data-val='FREQUENCY']").first().find(".lft-popup-ele-label[data-val='Quarterly+']").parent().hasClass("lft-popup-ele_active")) {
                        resolve('Successful');
                    } else {
                        if ($(".master-lft-ctrl[data-val='Measures']").find(".lft-popup-ele_active .lft-popup-ele-label").parent().parent().attr("first-level-selection") == "Brand Health Metrics")
                        {
...
function groupedColumnChart(data) {
    ...
    if($('.measure-text').text()=="Satisfaction Drivers - Top 2 box")
        data = prepareDataForGroupedColumnChart(data, 1);
    else
        data = prepareDataForGroupedColumnChart(data, 0);
    ...
    //ageNames = d3.keys(data[0]).filter(function (key) { return (key !== "State") && !(key.includes("__labelClass")) && (key !== "__sampleSize") && !(key.includes("__nullIdentifier")) && !(key.includes("__change")) && (key != "__lowSampleSize"); });
    data.forEach(function (d) {
        d.ages = ageNames.map(function (name) {
            return {
                name: name, value: +d[name], labelClass: d[name + "__labelClass"],sampleSize:+d[name + "__sampleSize"]
            };
        });
    });
    ...
    data = $(data).each(function (i, d) {
            $(d.ages).each(function (i, c) {
                c["groupName"] = d.name + '(' + c.sampleSize + ')';
            });
    });

    data = $(data).each(function (i,d) {
        $(d.ages).each(function (i, c) {
            c["value"] = (c["sampleSize"] < 30) ? null : c["value"];
        });
    });
    ...
    var state = svg.selectAll(".state")
      ...
      .enter().append("g")
        .attr("class", function (d) {
            return "state _" + d.__lowSampleSize;
        })
      ...
      .enter().append("rect")
        ...
        .attr("height", function (d, i) {
            ...
            context.append("text").attr("class", "upper-text" + i)
                ...
                .text(function () {
                    return d.value == null ? "" : d.value.toFixed(1) + "%";
                })
                .style("font-size", "10px")
...
function groupedBarChart(data) {
    ...
    data.forEach(function (d) {
        d.ages = ageNames.map(function (name) { return { name: name, value: +d[name], labelClass: d[name + "__labelClass"], sampleSize: +d[name + "__sampleSize"] }; });
    });
    var cummul_width = 0, flag = 0;
    ageNames.forEach(function (d, i) {
        cummul_width = cummul_width + ageNames[i].length * 8;
    ...
    data.forEach(function (d) {
        d.ages = temp_ageNames.map(function (name) { return { name: name, value: +d[name], labelClass: d[name + "__labelClass"], sampleSize: +d[name + "__sampleSize"] }; });
    });
    var temp_data = data.filter(function (d) { return d.sampleSize < 30 ? false : true; });
    if (data.length != 0) {
    ...
    var state = svg.selectAll(".state")
        .data(data)
      .enter().append("g")
        .attr("class", function (d) { return "state _" + d.__lowSampleSize; })
...
function groupedBarChartWithChange(data) {
    ...
    //var ageNames = d3.keys(data[0]).filter(function (key) { return (key !== "State") && !(key.includes("__labelClass")) && (key !== "__sampleSize") && !(key.includes("__nullIdentifier")) && !(key.includes("__change")) && (key != "__lowSampleSize"); });
    data.forEach(function (d) {
        d.ages = ageNames.map(function (name) { return { name: name, value: +d[name], labelClass: d[name + "__labelClass"], change: d[name + "__change"], sampleSize: +d[name + "__sampleSize"] }; });
    });
    var cummul_width = 0, flag = 0;
    ageNames.forEach(function (d, i) {
        cummul_width = cummul_width + ageNames[i].length * 8;
    ...
    data.forEach(function (d) {
        d.ages = ageNames.map(function (name) {
            return {
                name: name, value: +d[name], labelClass: d[name + "__labelClass"], change: d[name + "__change"], sampleSize: +d[name + "__sampleSize"]
            };
        });
    });
    //Add attribute name to each individual metrics
...
function Pyramid_Chart(data, isChange) {
    ...
    var color = colorPyramidStart;
    var colorForTopRect = colorPyramidStop;
...
function prepareDataForBarChart(data) {
    ...
    for (i = 0; i < data.length; i++) {
        for (j = 0; j < data[i].data.length; j++) {
            ...
            series["__sampleSize"] = data[i].data[j].SampleSize;
    ...
    for (i = 0; i < data.length; i++) {
        for (j = 0; j < data[i].data.length; j++) {
            ...
            if (data[i].data[j].change == undefined || data[i].data[j].change == null) {
                ...
                if ($('.table-statlayer').find('.activestat').text().trim() == "PREVIOUS PERIOD")
...
function prepareDataForGroupedColumnChart(data,isSatiDrivTop2Box) {
    var chartData = [];
    series = {};
    //[ {  "State": "CA", "Under 5 Years": 30,   "5 to 13 Years": 44,    "14 to 17 Years": 215,    "18 to 24 Years": 385,   "25 to 44 Years": 106,  "45 to 64 Years": 88,    "65 Years and Over": 41}];    
    var index = getMaxCount(data);
    for (j = 0; j < data.length; j++) {
        if (isSatiDrivTop2Box == 0) {
            series.State = data[j].name + "(" + (data[j].SeriesSampleSize == null ? "NA" : data[j].SeriesSampleSize) + ")";
        }
        else { series.State = data[j].name + "(NA)"; }
        series.name = data[j].name;
        ...
        for (i = 0, k = 0; i < data[index].data.length; i++, k++) {
            ...
            if (data[j].data[k] != undefined && data[j].data[k].StatValue != undefined && data[j].data[k].StatValue != null && data[index].data[k].x != null) {
                ...
                if (series[data[index].data[k].x + "__labelClass"] != "blue") {
                    ...
                    //for satisfaction drivers top 2 box, color based on individual samplesize
                    if (isSatiDrivTop2Box) {
                        if(data[index].data[k].SampleSize >= 30 && data[index].data[k].SampleSize<100)
                            series[data[index].data[k].x + "__labelClass"] = "gray";
                    }
            ...
            //sample size
            series[data[index].data[k].x + "__sampleSize"] = data[j].data[i].SampleSize;
            //sample size
        }
        chartData.push(angular.copy(series));
    }
...
var treeXTable = function (data) {
    ...
    row.push({ x: selectedFrequency, samplesize: "", significance: "", css: "grey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "" ,MetricSampleSize:""});
    if (checkFlag == 1) {
        for (var i = 0; i < data[0].data.length; i++) {
            row.push({ x: data[0].data[i].x, samplesize: "", significance: "", css: "darkgrey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
        }
    } else {
        for (var i = 0; i < data.length; i++) {
            row.push({ x: data[i].name, samplesize: "", significance: "", css: "darkgrey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: data[i].data[0].SampleSize });
        }
    }
    ...
    //sample size
    row = [];
    row.push({ x: "Sample Size", samplesize: "", significance: "", css: "grey font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    if (checkFlag == 1) {
        for (var index = 0; index < data[0].data.length; index++) {
            ...
            row.push({ x: temp_sampleSize == undefined || temp_sampleSize == null ? "NA" : sampleSizeStatus(temp_sampleSize), samplesize: "", significance: "", css: "grey font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
        }
    } else {
        for (var index = 0; index < data.length; index++) {
            row.push({ x: data[index].SeriesSampleSize == undefined || data[index].SeriesSampleSize == null ? "NA" : sampleSizeStatus(data[index].SeriesSampleSize), samplesize: "", significance: "", css: "grey font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
        }
    }
    newtabledata.push(row);
    // Selected Measure Row
    row = [];
    row.push({ x: txt, samplesize: "", significance: "", css: "tbl-dta-rowscolr", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    if (checkFlag == 1) {
        for (var index = 0; index < data[0].data.length; index++) {
            row.push({ x: "", samplesize: "", significance: "", css: "tbl-dta-rowscolr", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
        }
    } else {
        for (var index = 0; index < data.length; index++) {
            row.push({ x: "", samplesize: "", significance: "", css: "tbl-dta-rowscolr", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    ...
    if (checkFlag == 1) {
        for (var rowlength = 0; rowlength < data.length; rowlength++) {
            ...
            for (var i = 0; i < data[rowlength].data.length; i++) {
                if (data[0].data[i].SeriesSampleSize == "" || data[0].data[i].SeriesSampleSize == null)
                    row.push({ x: "NA", samplesize: "", significance: "", css: "grey", dataCss: getFontColorBasedOnStatValue(data[rowlength].data[i].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                else if (data[0].data[i].SeriesSampleSize > 99)
                    row.push({ x: data[rowlength].data[i].y === "" || data[rowlength].data[i].y == null ? "0%" : data[rowlength].data[i].y == undefined ? "0%" : data[rowlength].data[i].y.toFixed(1) + '%', samplesize: "", significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[rowlength].data[i].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                else if (data[0].data[i].SeriesSampleSize >= 30 && data[0].data[i].SeriesSampleSize <= 99)
                    row.push({ x: data[rowlength].data[i].y == "" || data[rowlength].data[i].y == null ? "0%" : data[rowlength].data[i].y == undefined ? "0%" : data[rowlength].data[i].y.toFixed(1) + '%', samplesize: "", significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[rowlength].data[i].StatValue), useDirectionallyCss: "yellowcolor", useDirectionallyForCirleCSS: "yellowcolorcircle", useDirectionallyFortdCSS: "yellowcolortd", MetricSampleSize: "" });
                    ...
                    row.push({ x: data[rowlength].data[i].y === "" || data[rowlength].data[i].y == null ? "0%" : data[rowlength].data[i].y == undefined ? "0%" : data[0].data[i].SeriesSampleSize < 30 ? "&nbsp;" : data[rowlength].data[i].y.toFixed(1) + '%', samplesize: "", significance: "", css: "grey", dataCss: getFontColorBasedOnStatValue(data[rowlength].data[i].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
            }
            newtabledata.push(row);
        }
    } else {
        for (var rowlength = 0; rowlength < data[0].data.length; rowlength++) {
            row = [];
            row.push({ x: data[0].data[rowlength].x, samplesize: data[0].data[rowlength].SampleSize, significance: "" });
            for (var i = 0; i < data.length; i++) {
                if (data[i].SeriesSampleSize == "" || data[i].SeriesSampleSize == null)
                    row.push({ x: "NA", samplesize: "", significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[i].data[rowlength].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                else if (data[i].SeriesSampleSize > 99)
                    row.push({ x: parseInt(data[i].data[rowlength].y) === "" || data[i].data[rowlength].y == null ? "NA" : data[i].data[rowlength].y == undefined ? "0%" : data[i].data[rowlength].y.toFixed(1) + '%', samplesize: data[i].data[rowlength].SampleSize, significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[i].data[rowlength].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: data[i].data[rowlength].SampleSize });
                ...
                {
                    if ((data[i].data[rowlength].x == "Food" || data[i].data[rowlength].x == "Beverages" || data[i].data[rowlength].x == "Value" || data[i].data[rowlength].x == "Service" || data[i].data[rowlength].x == "Atmosphere" || data[i].data[rowlength].x == "Cleanliness")) {
                        if (data[i].data[rowlength].sampleSize < 30 )
                            var x = "&nbsp;";
                        else
                            var x=  parseInt(data[i].data[rowlength].y) === "" || data[i].data[rowlength].y == null ? "NA" : data[i].data[rowlength].y == undefined ? "0%" : data[i].data[rowlength].y.toFixed(1) + '%';
                    }
                    else
                        var x = "&nbsp;";
                    row.push({ x: x, samplesize: "", significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[i].data[rowlength].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: data[i].data[rowlength].SampleSize });
                } else
                    row.push({ x: parseInt(data[i].data[rowlength].y) === "" || data[i].data[rowlength].y == null ? "NA" : data[i].data[rowlength].y == undefined ? "0%" : "NA", samplesize: "", significance: "", css: "", dataCss: getFontColorBasedOnStatValue(data[i].data[rowlength].StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: data[i].data[rowlength].SampleSize });
            }
            newtabledata.push(row);
        }
    }
...
var treeXTableForCompareTrend = function (data, Esttext) {
    ...
    var selectedFrequency = $('.FREQUENCY_topdiv_element').find('.sel_text').text();
    headerrow.push({ x: selectedFrequency, samplesize: "", significance: "", css: "grey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });

    for (var i = 0; i < data.length; i++) {
        headerrow.push({ x: data[i].name, samplesize: "", significance: "", css: "darkgrey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    }
    newtabledata.push(headerrow);
    //Empty sample size row
    headerrow = [];
    headerrow.push({ x: "", samplesize: "", significance: "", css: "gray font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    for (var i = 0; i < data.length; i++) {
        headerrow.push({ x: "", samplesize: "", significance: "", css: "darkgrey", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    }
    newtabledata.push(headerrow);
    //Sub header
    headerrow = [];
    headerrow.push({ x: Esttext, samplesize: "", significance: "", css: "tbl-dta-rowscolr", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    for (var i = 0; i < data.length; i++) {
        headerrow.push({ x: "", samplesize: "", significance: "", css: "tbl-dta-rowscolr", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
    }
    newtabledata.push(headerrow);
    ...
    for (var i = 0; i < data[0].data.length; i++) {
        ...
        $.each(data, function (a, b) {
            ...
            if (index == 0) {
                sampleSizerow = [];
                sampleSizerow.push({ x: "Sample Size", samplesize: "", significance: "", css: "grayNotImp bold font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                sampleSizerow.push({ x: d.SampleSize == "" || d.SampleSize == null ? "NA" : d.y == undefined ? "NA" : sampleSizeStatus(d.SampleSize), samplesize: "", significance: "", css: "grayNotImp bold", dataCss: "", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                ...
                establishmentRow.push({ x: parseInt(d.y) === "" || d.y == null ? "NA" : d.y == undefined ? "NA" : d.y.toFixed(1) + '%', samplesize: "", significance: "", css: "", dataCss: getUseDirectionalColor(sampleSizeColor, d.StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
            }
            else {
                ...
                sampleSizerow.push({ x: d.SampleSize == undefined || d.SampleSize == null ? "NA" : sampleSizeStatus(d.SampleSize), samplesize: "", significance: "", css: "grayNotImp bold font11", dataCss: "black", useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
                establishmentRow.push({ x: parseInt(d.y) === "" || d.y == null ? "NA" : d.y == undefined ? "NA" : d.y.toFixed(1) + '%', samplesize: "", significance: "", css: "", dataCss: getUseDirectionalColor(sampleSizeColor, d.StatValue), useDirectionallyCss: "", useDirectionallyForCirleCSS: "", useDirectionallyFortdCSS: "", MetricSampleSize: "" });
            }
            index++;
        });
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (procName == "CreateReport")
                        showMaxAlert("Report is Created");
                    else if (procName == "EditReport") {
                        showMaxAlert(response);
                    }
                    else
                        showMaxAlert("Slide is added to report");
                    $(".saveAsPopup").hide();
                    $(".save-reportPopup").hide();
...
/*Start for color pallet*/
$(document).on("click", ".color-pallet-chart", function () {
    $(".custom-color-palte .heading_text").text("SELECT COLOR");
    $('.customcolor-content').html("");
    var dataVal = getDataValforColor();
    var estblselectedLst = $('.filter-info-panel-elements .topdiv_element[data-val="' + dataVal + '"]').find('.filter-info-panel-lbl');
    var customcolorhtml = "";
    $.each(estblselectedLst, function (i, d) {
        ...
        customcolorhtml += "<div class='custom-estcolordiv' id='" + $(d).attr("data-id") + "' ></div>";
        customcolorhtml += "</div>";
        customcolorhtml += "</div>";
    });
    $(".customcolor-content").append(customcolorhtml);
    //Assign colors to All list
    assignfillcolorinpopup(dataVal);
    $(".stat-popup").hide();
    $(".color-pallet-chart").css("background-position", "-2283px -408px");
    $(".custom-color-palte, .transparentBG").show();
    SetScroll($(".customcolor-content"), "#ffffff", 0, 15, 0, -8, 8);
});

var CallvalidateColorCode = function () {
    validateColorCode(getDataValforColor());
}

var getDataValforColor = function () {
    var dataValRes = "Measures";
    var ismultiplemeasures = $(".Measures_topdiv_element .sel_text").length > 1;
    if ($(".trend.active").length == 1) {
        switch (controllername) {
            case "chartbeveragecompare": dataValRes = "Beverage"; break;
            case "chartbeveragedeepdive": if (!ismultiplemeasures) { dataValRes = "Metric Comparisons"; } break;
            case "chartestablishmentcompare": dataValRes = "Establishment"; break;
            case "chartestablishmentdeepdive": if (!ismultiplemeasures) { dataValRes = "Metric Comparisons"; } break;
        }
    }
    else {
        switch (controllername) {
            case "chartestablishmentcompare": if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") { dataValRes = "Establishment"; } break;
            case "chartestablishmentdeepdive": if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") { dataValRes = "Metric Comparisons"; } break;

            case "chartbeveragecompare": if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") { dataValRes = "Beverage"; } break;
            case "chartbeveragedeepdive": if (active_chart_type == "pyramidwithchange" || active_chart_type == "pyramid") { dataValRes = "Metric Comparisons"; } break;
        }
    }
    return dataValRes;
}
/*End for color pallet*/